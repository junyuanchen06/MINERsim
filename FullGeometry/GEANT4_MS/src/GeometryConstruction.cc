...
G4AssemblyVolume* GeometryConstruction::ConstructBEGe(std::string name)
{
  ...
  G4double detRad = 50.8*mm; 
  G4double detHalfZ = 16.891*mm; 

  G4VSolid* target = new G4Tubs("Target"+name,0.,2.5*cm,(4.805/2.)*cm,0,360*deg);
  ...
  G4Tubs *liqN2_inTubeA = new G4Tubs("liqN2_inTubeA"+name, 0., tubeAdia/2. - tubeAthick, tubeAlen/2. - tubeAthick/2.,0,360*deg);
  G4SubtractionSolid *liqN2_TubeA = new G4SubtractionSolid("liqN2_TubeA"+name,liqN2_inTubeA,target,0,-1*tubeA_N2_Pos);
  G4Tubs *liqN2_TubeB = new G4Tubs("liqN2_TubeB"+name, 0., tubeBdia/2. - tubeBthick, tubeBlen/2. - tubeBthick/2.,0,360*deg);
  G4Tubs *liqN2_TubeC = new G4Tubs("liqN2_TubeC"+name, 0., tubeCdia/2. - tubeCthick, tubeClen/2.,0,360*deg);
  ...
  G4LogicalVolume *fLogicDetLiN2A = new G4LogicalVolume(liqN2_TubeA,mats->GetLiN2(),"liqN2_TubeA_log"+name);
  G4LogicalVolume *fLogicDetLiN2B = new G4LogicalVolume(liqN2_TubeB,mats->GetLiN2(),"liqN2_TubeB_log"+name);
  G4LogicalVolume *fLogicDetLiN2C = new G4LogicalVolume(liqN2_TubeC,mats->GetLiN2(),"liqN2_TubeC_log"+name);
...
//  G4ThreeVector   brick4_place(0.,-1*(0.5*(brick_y + wgGap) + 0.5*(brick_x + wgGap)),0.);
  ...
  G4ThreeVector posDet = G4ThreeVector(water_rad+bioShield_thick - thermalC_length_big - thermalC_length_small + 66.*in + 8.5*in + 0.5*detHalfZ + (1./16.)*in+2.*in,-5.5*in,-(worldZ/2.)+floor_thick_out+floor_to_TC_out-3.5*in);

  G4ThreeVector posDet2 = G4ThreeVector(water_rad+bioShield_thick - thermalC_length_big - thermalC_length_small + 66.*in + 0.5*detHalfZ + (1./16.)*in+2.*in,-5.5*in,-(worldZ/2.)+floor_thick_out+floor_to_TC_out+13.5*in);
